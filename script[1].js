...
function applyConfig() {
  document.querySelectorAll("#dogNameInline, #dogNameInline2").forEach(el => {
...
function onPointerDown(e) {
  ...
  missionCaption.textContent = "Continua…";
}
...
function onPointerUp() {
  ...
  dragState = null;
  if (!missionComplete) {
    missionCaption.textContent = "Quasi. Trascinalo ancora un po' →";
  }
}
...
function spawnConfetti() {
  const colors = [
    "var(--gold)", "var(--sage)", "var(--azzurro)",
    "var(--gold-deep)", "var(--sage-deep)", "var(--azzurro-deep)",
  ];
  const originX = dogFrame.getBoundingClientRect().left - stage.getBoundingClientRect().left + dogFrame.offsetWidth / 2;
  const originY = dogFrame.getBoundingClientRect().top - stage.getBoundingClientRect().top + dogFrame.offsetHeight * 0.2;

  const count = 34; // più coriandoli, per un'esplosione più festosa
  for (let i = 0; i < count; i++) {
    ...
    const ty = Math.sin(angle) * distance - 55; // spinta più decisa verso l'alto
    ...
    piece.style.setProperty("--confetto-radius", Math.random() > 0.5 ? "50%" : "2px");
